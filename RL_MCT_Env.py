    ...
    def step(self, action):
        ...
        if self.action_idx in [1,2,3,5,6,7]: 
            ...
            if barrier > 5:
                ...
                # reward += math.tanh(-relative_energy /(self.H * 8.314 * self.temperature_K)) * (math.pow(10.0, 5))
                reward += -relative_energy / (self.H * self.k * self.temperature_K)
...
#        reward -= 0.5 # 每经历一步timesteps，扣一分
        ...
        if len(self.history['real_energies']) > 11:
            ...
            if RMSE < self.min_RE_d * 2 / 3:
                done_similar = True

        if ((current_energy - self.initial_energy) <= -0.95 * self.H and (abs(current_energy - previous_energy) < self.min_RE_d and abs(current_energy - previous_energy) > 0.0001)) and ((current_energy - self.initial_energy) <= -0.90 * self.H and done_similar):   # 当氧气全部被吸附到Pd表面，且两次相隔的能量差小于一定阈值，达到终止条件
        ...
        if self.done:
            episode_over = True
            self.episode += 1
            if self.episode % self.save_every == 0 or done_similar:
                self.save_episode()
                self.plot_episode()
    ...
    def get_surf_sites(self, slab):
        ...
        env_2 = self.label_atoms(state, [0.0,  4.0])
        for i in env_2:
            envList.append(i)
    ...
    def to_expand_lattice(self, slab, expand_layer, expand_surf, expand_lattice):
        ...
        for i in slab:
            ...
            '''if i.index in subList:
                slab.positions[i.index][0] = (slab.get_positions()[i.index][0] - mid_point_sub[0]) * expand_sub + \
                                             mid_point_sub[0]
                slab.positions[i.index][1] = (slab.get_positions()[i.index][1] - mid_point_sub[1]) * expand_sub + \
                                             mid_point_sub[1]'''
        return slab
    ...
    def to_constraint(self, slab):
        ...
        envList_2 = self.label_atoms(slab, [0.0,  4.0])
        constraint = FixAtoms(mask=[a.symbol != 'O' and a.index in bottomList for a in slab])
...
envList = label_atoms(slab, [2.0- fluct_d_layer, 2.0 + fluct_d_layer])
env = slab.copy()
del env[[i for i in range(len(env)) if i not in envList]]
