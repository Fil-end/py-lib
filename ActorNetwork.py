...
class PaiNNPolicyNet(nn.Module):
    ...
    def __init__(self, r_state_dim, r_hidden_dim, r_action_dim, node_size):
        super().__init__()
        ...
        self.deep_to_rein_mlp = nn.Sequential(
            ...
            nn.SiLU(),
            nn.Linear(r_hidden_dim, r_action_dim),
        )
    ...
    def forward(self, node_state_scalar, node_state_vector):
        ...
        '''
        pre_linear = self.pre_linear(node_state_vector)  # num_nodes, 3, node_size

        if len(node_state_scalar.shape) == 2:
            pre_norm = torch.linalg.norm(pre_linear, dim=1, keepdim=False)  # num_nodes, node_size
            mlp_input = torch.cat(
                (node_state_scalar, pre_norm), dim=1
            )  # num_nodes, node_size*2
            
            mlp_output = self.combined_mlp(mlp_input)
            mlp_output = rearrange(mlp_output, 'a b -> b a') # len(transition_dict),num_nodes, node_size * 2 -> node_size*2, num_nodes

            # print('torch.mean(F.softmax(self.deep_to_rein_mlp(mlp_output), dim=1),dim = 0)',torch.mean(F.softmax(self.deep_to_rein_mlp(mlp_output), dim=1),dim = 0))   
            return torch.mean(F.softmax(self.deep_to_rein_mlp(mlp_output), dim=1),dim = 0)
        
        elif len(node_state_scalar.shape) == 3:
            ...
            mlp_output = self.combined_mlp(mlp_input)
            mlp_output = rearrange(mlp_output, 'a b c ->a c b') # len(transition_dict), num_nodes, node_size * 2 -> len(transition_dict), node_size*2, num_nodes

            print('action_probs',torch.mean(F.softmax(self.deep_to_rein_mlp(mlp_output), dim=2),dim = 1).shape)

            return torch.mean(F.softmax(self.deep_to_rein_mlp(mlp_output), dim=2),dim = 1)